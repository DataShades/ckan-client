...
const store = writable({items: new Map<string, [TDataset, TResource]>(), processing: false});
...
const fullUpload=  () => {
  ...
  get(Source).datasets.forEach(d => d.resources.filter(r => {
...
const exhaustQueue = async () => {
  ...
  while (true) {
    ...
    if (!upload) {
      ...
      add(dataset, resource)
      await Submission.progressUpload(dataset.name, resource.name, upload.data.chunks_uploaded + 1)
    } else {
